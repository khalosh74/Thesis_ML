...
- Resolve estimator construction through `experiments/backend_registry.py`; in PR 4 exploratory `ridge` and `logreg` can resolve to `torch_gpu` when `gpu_only` capability is valid, while other models remain CPU reference.
...
- Validate official compute controls conservatively before dispatch (`cpu_only` only in PR 4 scope).
...
- run artifacts also persist additive compute-policy metadata (`hardware_mode_*`, backend selection/fallback metadata, GPU device metadata when available); in PR 4 exploratory `ridge`/`logreg` may execute through `torch_gpu` while official paths remain CPU-only.
