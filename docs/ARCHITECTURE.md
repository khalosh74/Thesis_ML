...
- run artifacts also persist additive stage-execution metadata (`stage_execution.policy`, `stage_execution.assignments`, `stage_execution.telemetry`) from the Phase 1 run-level bridge in `experiments/stage_execution.py`; this is structural/observability-only and does not change scheduling or scientific behavior.
