...
def test_aliases_resolve_to_expected_ground_truth_paths() -> None:
    ...
    assert _resolve_alias("target.coarse_affect_default") == (
        repo_root / "configs" / "targets" / "affect_mapping_v2.json"
    ).resolve()
    assert _resolve_alias("protocol.thesis_canonical_default") == (
        repo_root / "configs" / "protocols" / "thesis_canonical_nested_v2.json"
    ).resolve()
    assert _resolve_alias("protocol.thesis_confirmatory_frozen") == (
        repo_root / "configs" / "protocols" / "thesis_confirmatory_v1.json"
    ).resolve()
    assert _resolve_alias("comparison.grouped_nested_default") == (
        repo_root / "configs" / "comparisons" / "model_family_grouped_nested_comparison_v2.json"
    ).resolve()
    assert _resolve_alias("registry.decision_support_default") == (
        repo_root / "configs" / "decision_support_registry.json"
    ).resolve()
