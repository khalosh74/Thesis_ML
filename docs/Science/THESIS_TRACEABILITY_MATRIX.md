...
| D-03 | Primary validation setting | Use within-person held-out-session validation as the primary test of stable predictive signal. | `configs/protocols/thesis_canonical_nested_v2.json` | RQ-01, C-01 | Participant-specific fold manifest; primary score tables. | locked |
| D-04 | Secondary transfer setting | Evaluate cross-person transfer separately using frozen parameters and no re-training. | `configs/protocols/thesis_canonical_nested_v2.json` | RQ-02, C-01 | Frozen transfer results; comparison table. | locked |
...
| D-06 | Baseline / control policy | Use appropriate baselines and permutation-style controls where applicable to distinguish real signal from chance or design artifacts. | `configs/protocols/thesis_canonical_nested_v2.json` | RQ-01, C-03 | Baseline table; permutation summary; chance-reference note. | locked |
...
| D-08 | Model comparison frame | Compare models under the same locked split logic and preprocessing discipline; do not treat architecture novelty as the main contribution. | `configs/comparisons/model_family_grouped_nested_comparison_v2.json` | C-02 | Model comparison table; run manifest. | locked |
...
|---|---|---|---|---|---|
| PROT-WP-PRIMARY | Primary within-person confirmatory evaluation | Train and test must be separated at the session level; preprocessing and any tuning must remain within training data for each split. | `ART-WP-01` participant-level score table; `ART-WP-02` aggregated score table; `ART-WP-03` confusion patterns; `ART-WP-04` fold manifest. | RQ-00, RQ-01, C-01, C-03 | planned |
| PROT-XFER-FROZEN | Secondary cross-person transfer evaluation | Train on participant A and apply frozen model to participant B, and vice versa, with no re-training on the target participant. | `ART-XFER-01` A->B table; `ART-XFER-02` B->A table; `ART-XFER-03` comparison against within-person reference. | RQ-02, C-01, S-01 | planned |
| PROT-BASELINE-CONTROL | Chance-reference and control evaluation | Predeclare baseline and permutation-style controls consistent with the same split logic as the primary analysis. | `ART-CTRL-01` baseline results; `ART-CTRL-02` permutation summary; `ART-CTRL-03` interpretive note. | RQ-01, C-03 | planned |
| PROT-INTERP-STABILITY | Interpretability and robustness analysis | Compute model-behavior explanations only from locked models/runs and summarize whether patterns are directionally plausible and reasonably stable across held-out folds or sessions. | `ART-INT-01` global importance summary; `ART-INT-02` fold/session stability summary; `ART-INT-03` interpretation caveat note. | RQ-03, C-01, S-02 | planned |
| PROT-REPRO-AUDIT | Reproducibility and audit package | Every confirmatory run must emit configuration, dataset fingerprint, software/environment info, and artifact manifest. | `ART-AUD-01` run manifest; `ART-AUD-02` config snapshot; `ART-AUD-03` dataset fingerprint; `ART-AUD-04` environment snapshot. | C-01, C-03, TH-3.6, TH-5 limitations/reproducibility | planned |
| PROT-SCOPE-GUARDRAIL | Scope and non-claim enforcement | Result templates and discussion templates must preserve the thesis scope and non-claims. | `ART-SCOPE-01` reporting template; `ART-SCOPE-02` limitations template; `ART-SCOPE-03` non-claim checklist. | S-01, S-02, TH-5 Discussion | planned |
