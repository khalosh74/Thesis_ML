...
def _resolve_pipeline_components(
    ...
    if not isinstance(scaler, StandardScaler):
        raise ValueError(
            "Specialized linearsvc tuning requires pipeline step 'scaler' to be StandardScaler."
        )
    if not bool(getattr(scaler, "with_mean", False)) or not bool(getattr(scaler, "with_std", False)):
        raise ValueError(
            "Specialized linearsvc tuning requires StandardScaler(with_mean=True, with_std=True)."
        )
