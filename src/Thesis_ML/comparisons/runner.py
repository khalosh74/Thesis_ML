...
def _source_comparison_identity(comparison: ComparisonSpec) -> dict[str, Any]:
...
def _validate_official_comparison_gpu_admission(
    *,
    ...
    if disallowed_runs or unsupported_runs:
        ...
        raise ValueError(
            "Official locked comparison gpu_only admission rejected. "
...
def execute_compiled_comparison(
    *,
    ...
    for order_index, pending in enumerate(pending_run_executions):
        spec = pending["spec"]
        if not isinstance(spec, CompiledComparisonRunSpec):
            raise TypeError("Comparison pending execution produced invalid spec entry.")
        run_dir = pending["run_dir"]
        if not isinstance(run_dir, Path):
            raise TypeError("Comparison pending execution produced invalid run directory entry.")
    ...
    for dispatched in dispatch_results:
        ...
        if metadata is None:
            raise RuntimeError("Comparison parallel execution returned unknown job order index.")
        spec = metadata["spec"]
        if not isinstance(spec, CompiledComparisonRunSpec):
            raise TypeError("Comparison parallel execution metadata contains invalid spec entry.")
        run_dir = metadata["run_dir"]
        if not isinstance(run_dir, Path):
            raise TypeError(
                "Comparison parallel execution metadata contains invalid run directory entry."
            )
    ...
    source_comparison = _source_comparison_identity(comparison)
